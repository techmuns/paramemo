...
async function handleGenerate(request, env, ctx) {
  ...
  const system =
    ...
    'SOURCE PRECEDENCE (when documents disagree): a company\'s OWN IM / teaser / company deck is the PRIMARY source for facts about itself — its team\'s credentials, its metrics, its history. A banker call note is SECONDARY and often generalises, so it must NOT overwrite a SPECIFIC fact the IM states. In particular, when the IM\'s team / leadership slide gives a person\'s exact education or background, use THAT for that person — a note\'s sweeping claim (e.g. "the founders are IIT 2016 batch") must NOT replace the slide\'s per-person detail (e.g. a founder shown as "B.E. Chemical, BITS Pilani"). Where an IM fact and a note genuinely conflict on a person\'s college, a revenue figure, or a stake, follow the IM/company document and, if the gap is material, note the discrepancy — never silently pick the note, and never blend the two into a claim neither source actually makes. Apply this in EVERY field you write — snapshot, the fitChecklist notes, the thesis, concerns and any one-line summary included: e.g. if the IM team slide shows the founders at BITS Pilani, do NOT write "IIT" anywhere (not in a checklist note, not in a summary), even where a banker note says "IIT".\n\n' +
    ...
    '• YEAR SPAN — use the LONGEST labelled series in the documents; do NOT truncate to a single Excel tab. If the IM has a financial-summary page whose headers carry A / E / P suffixes (e.g. "FY22A FY23A FY24A FY25A FY26E … FY31P"), financials.years MUST span that FULL range — you MUST include the earlier ACTUAL years it shows (FY22A, FY23A, FY24A) EVEN WHEN the Excel projection tab you take row detail from only begins at a later year (e.g. FY25). Never shorten the reported history to where the Excel tab starts. For those earlier actual years fill revenue / EBITDA / growth from the IM financial page and leave rows the page does not give (segments, balance sheet) as null; copy the year LABELS verbatim.\n' +
    '• A year is a FORECAST if it is marked estimate/projected/budget/guidance (suffix E, P, Est, Proj, Bud, or a "guidance"/"projected" label) or the model clearly presents it as such; a year shown as a reported/audited ACTUAL (suffix A, or a plain reported figure the CIM/notes treat as already achieved) is an ACTUAL.\n' +
    '• financials.actualsThrough = the LAST ACTUAL (non-forecast) year, exactly matching the documents\' own actual-vs-forecast split (e.g. if the IM labels FY25A actual and FY26E an estimate, actualsThrough is FY25; if the banker notes report a year as an achieved actual and only the NEXT year as "guidance", that reported year is the last actual). Do not mark an actual year as an estimate or vice-versa.\n' +
    ...
    '• MONEY UNITS — READ THE SOURCE UNIT, NEVER ASSUME (this is the #1 source of error). Each Excel sheet states its OWN reporting unit — look at the "reporting unit:" note on the "# Sheet:" line and any "Currency: …" / "INR …" / "Amount in …" cell. Convert EVERY money value to ₹ crore using the CORRECT factor for THAT sheet\'s unit: **₹ crore → KEEP AS-IS (×1, do NOT divide)**; ₹ million/mn → ÷10; ₹ lakh/lac → ÷100; ₹ thousand/\'000 → ÷10,000; absolute ₹ → ÷1,00,00,000. A workbook may MIX units across tabs (e.g. a consolidated tab in ₹ crore and detail tabs in ₹ lakh) — use EACH figure\'s own sheet unit, and prefer the CONSOLIDATED / group P&L for the totals. Keep ONE decimal place (e.g. 214 mn → 21.4). Keep the model\'s full year span (often 6–10 years).\n' +
    '• UNIT SANITY CHECK (MANDATORY — prevents the 10×/100× mistakes): after converting, your latest-actual revenue in ₹ crore MUST be the same order of magnitude as any revenue the CIM / teaser / banker notes state IN WORDS (e.g. "FY26 revenue ₹385 cr", "₹42 cr/month run-rate ≈ ₹500 cr"). If your number comes out ~10× or ~100× different from the document\'s stated figure, you mis-read the unit — RE-CONVERT until they line up. Apply the same check to EBITDA and to the transaction/ask size. NEVER publish financials that contradict the document\'s own stated headline by a clean factor of 10.\n' +
    '• financials.unit = "₹ crore" and financials.unitSource = the sheet + unit you took the group totals from, e.g. "Consolidated_IS · INR Cr" — so a partner can see where the scale came from.\n' +
    ...
    'FIGURES SELF-AUDIT (ALWAYS include "figuresAudit" — this is the tripwire that catches unit / scale mistakes before they reach the partner):\n' +
    '• figuresAudit = { unitSource:"<the sheet + unit you used for the group P&L, e.g. \'Consolidated_IS · INR Cr\'>", latestActualYear:"<e.g. FY26>", modelRevenueCr:<your converted revenue for that year, ₹ cr>, imStatedRevenueCr:<the revenue the CIM/teaser/banker notes state for that year IN WORDS, in ₹ cr, or null if none stated>, modelEbitdaCr:<converted EBITDA that year, or null>, imStatedEbitdaCr:<stated, or null>, revenueConsistent:true|false, checks:[ "<short pass/flag lines, e.g. \'Revenue ₹385cr model vs ₹385cr CIM — OK\', or \'Ask ₹300cr vs ₹385cr revenue — 0.8x, OK\'>" ], note:"<one line; if revenueConsistent:false, state the scale gap you found and how you reconciled it>" }.\n' +
    '• Set revenueConsistent:false whenever modelRevenueCr and imStatedRevenueCr differ by a clean ~10× / ~100× — and FIX the financials so they line up BEFORE returning (do not knowingly return inconsistent numbers). If the documents state no revenue figure, leave imStatedRevenueCr null and say so in note.\n\n' +
    ...
    'COMPS — VALUATION COMPS (always attempt "comps" for a real operating company; omit only if you genuinely cannot identify any peer set). "comps" has three sub-blocks — build as many as you can:\n' +
    '• comps.peerBenchmark = { asOf, note, self:{ name, listed:false, revenueCr, revenueGrowthPct, ebitdaPct, patPct }, rows:[ { name, listed, ticker|null, revenueCr, revenueGrowthPct, ebitdaPct, patPct, note } ] } — OPERATING metrics (revenue, growth, margins), NOT valuation. Null any figure the documents don\'t give. Build this whenever the IM/notes name ANY competitors/peers — even with only NAMES and a one-line qualitative note (put it in each row\'s "note").\n' +
    '• comps.trading = { asOf:"<the model / report date or today>", source:"Listed peers · indicative multiples (model estimate — verify)", note, rows:[ { name, ticker:"<NSE/BSE code>", listed:true, marketCapCr, evCr, revenueCr, ebitdaPct, evEbitda, evRevenue, pe, note } ] } — TRADING COMPS: the sector\'s LISTED comparable companies and the multiples they TRADE at. Identify 4–10 GENUINE listed peers in the same space (Indian names preferred) with their REAL NSE/BSE tickers, and fill evEbitda / evRevenue / pe with your best APPROXIMATE public figures. These are INDICATIVE ESTIMATES from general knowledge — keep `source` exactly as given and NEVER describe them as "live", "as reported", or "refreshed live" (the app runs a SEPARATE live market-data pull that refreshes what it can). The companies and tickers MUST be real (never invent a ticker). Leave marketCapCr/evCr null unless the documents state them (they fill live).\n' +
    '• comps.transactions = { source:"<name the ACTUAL source of these rows — see PROVENANCE below>", note, rows:[ { date:"YYYY" or "YYYY-MM", target, buyer, seller|null, dealType:"Strategic"|"PE"|"Financial", region:"Domestic"|"Inbound"|"PE"|"Domestic (PE backed)", stakePct, dealValue:"<as quoted, e.g. ₹340 cr or $34 mn>", evEbitda, evRevenue, note:"<one line on the target>" } ] } — TRANSACTION COMPS: past M&A / PE deals for comparable companies and the multiples PAID. FIRST use the peers/deals the DOCUMENTS actually name — SCAN THE WHOLE document set, not just a section headed "transactions": an attached export, a competitive-landscape or precedent-transactions section, an international-benchmark / comparable-company / case-study slide, ANY "acquired by / bought by / merged with / raised / invested in" mention anywhere in the IM or notes, and the TARGET\'s OWN prior funding rounds and last valuation. A company a slide names as "Acquired by <X>" (or "<X> acquired <company>") IS a transaction comp — include it (date and dealValue as disclosed, else null), naming the target and buyer and labelling it by the slide it came from. These document-named deals come first; you may ADD well-known DISCLOSED deals from general knowledge only as clearly-labelled illustrative extras. NEVER invent a deal value or multiple that was not actually disclosed — null any undisclosed multiple, and omit any "deal" you are not confident really happened.\n' +
    '• PROVENANCE — SOURCES MUST BE TRUTHFUL (never dress a guess up as data): every comps/peers block\'s `source` must name where its rows ACTUALLY came from, and you must not claim a source you did not use. Rows taken from a document → cite it ("Banker notes · competitive landscape", "Company IM", "Private Circle export"). Rows you added from general knowledge → label them "publicly reported (illustrative)"; do NOT attribute them to the banker notes, the IM, or "live market data" when those did not contain them. If the documents name a specific competitor / peer set, USE those names first and keep any general-knowledge additions the clear minority.\n' +
    '• IF a PRIVATE CIRCLE / valuation-comps / CapitalIQ / VCCEdge export is attached (in the additional documents or page images), PARSE ITS TABLES DIRECTLY — its trading tab into comps.trading, its transactions tab into comps.transactions — mapping the columns to the fields above. This is the most reliable source, especially for PRIVATE names. All money in ₹ crore except transaction dealValue (keep it as quoted). Percentages are plain numbers (18 = 18%).\n' +
    '• EXHAUSTIVENESS — capture EVERY comparable the documents NAME, with no exceptions: Indian or global, listed or unlisted, a direct competitor or a company cited only as a benchmark, a case study, an acquirer, or a manager\'s former employer. Put each named OPERATING peer in comps.peerBenchmark, and each named DEAL / acquisition / merger / funding round (including the target\'s OWN past rounds) in comps.transactions. Never drop a name because it is foreign, private, or mentioned only once — the partner wants NO named peer missing from the Comps tab. Keep provenance honest (label document-named rows by their real source, general-knowledge rows as "publicly reported (illustrative)"), and still never invent a figure that was not disclosed (null it).\n\n' +
    ...
    'CLASSIFY people into the right list: FOUNDERS / CO-FOUNDERS / PROMOTERS — the people who started or control the company, often on a slide headed "Co-Founders", "Founders", "Promoters" or "Leadership" — go in snapshot.promoters; HIRED executives who are NOT founders (VPs, and CxOs brought in later, often under "Management team" / "Key management") go in snapshot.management. When a slide explicitly groups people as "Co-Founders", EVERY person in that group is a promoter — never file a founder under management, and never leave snapshot.promoters empty when the deck clearly names the founders. ' +
    ...
    '• entryYear = the year the deal is actually ENTERED / PRICED on. When the documents price the deal on a specific year\'s metric — e.g. a banker note stating "~5.2× FY26 revenue", or an ask quoted against a forward year — use THAT year (e.g. FY26E) as entryYear so entryX reproduces the stated multiple; do NOT default to the last historical actual (which would inflate the multiple, e.g. 8.9× on a trailing year instead of 5.2× on the priced-on year). Absent an explicit pricing year, use the latest actual (or nearest forward year) with a positive value on your chosen entryBasis — positive EBITDA for an EBITDA/PAT basis; any year with positive revenue for a revenue basis. startYear = entryYear and startEbitdaCr = that year\'s EBITDA in ₹ cr (still recorded even on a revenue basis; keep it positive and matching financials).\n' +
    ...
    '   RULE: if entering on EBITDA at the deal\'s stated valuation would require a multiple above ~30×, the deal is NOT EBITDA-priced — use "revenue" basis instead. When a valuation ask is stated, pick basis + entryX so the implied entry value is in the BALLPARK of that ask (entry metric × entryX ≈ the ask), not a generic multiple that ignores it. ENTRY AND EXIT BASIS CAN DIFFER — set them independently. A company priced on REVENUE today (thin/early EBITDA) is normally EXITED on an EARNINGS multiple once it matures: so when entryBasis is "revenue" but the model shows the company solidly profitable by the exit year (a healthy positive EBITDA margin, and positive PAT for a P/E exit), set exitBasis to "ebitda" (or "pat" for an earnings-led / consumer deal) and set exitX to a sane multiple on THAT basis for the exit year (EBITDA ~10–16×, P/E ~15–25×) — do NOT force the exit onto the entry\'s revenue basis. Keep entryBasis and exitBasis the SAME only when the company does not re-rate (still thin-margin at exit, or a normal profitable business entered and exited on EBITDA). growthPct = a plausible growth % anchored to the model; years = hold period; underdeliverPct = 0 (the partner raises it to stress-test).\n\n' +
...
function coerceComps(k) {
  ...
  if (isObj(k.trading) && Array.isArray(k.trading.rows)) {
    const tr = k.trading;
    tr.rows = tr.rows.filter(isObj).map(r => numRow(r, ['marketCapCr', 'evCr', 'revenueCr', 'ebitdaPct', 'evEbitda', 'evRevenue', 'pe']));
