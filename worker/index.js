...
async function handleGenerate(request, env, ctx) {
  ...
  const system =
    ...
    '• comps.trading = { asOf:"<the model / report date or today>", source:"Listed peers · public market data (refreshed live)", note, rows:[ { name, ticker:"<NSE/BSE code>", listed:true, marketCapCr, evCr, revenueCr, ebitdaPct, evEbitda, evRevenue, pe, note } ] } — TRADING COMPS: the sector\'s LISTED comparable companies and the multiples they TRADE at. A listed company\'s valuation is PUBLIC, so identify 4–10 GENUINE listed peers in the same space (Indian names preferred) with their REAL NSE/BSE tickers, and fill evEbitda / evRevenue / pe with your best current public figures — these refresh live from market data, so approximate is fine, but the companies and tickers MUST be real (never invent a ticker). Leave marketCapCr/evCr null unless the documents state them (they fill live).\n' +
    '• comps.transactions = { source:"<e.g. Private Circle / VCCEdge / disclosed deals>", note, rows:[ { date:"YYYY" or "YYYY-MM", target, buyer, seller|null, dealType:"Strategic"|"PE"|"Financial", region:"Domestic"|"Inbound"|"PE"|"Domestic (PE backed)", stakePct, dealValue:"<as quoted, e.g. ₹340 cr or $34 mn>", evEbitda, evRevenue, note:"<one line on the target>" } ] } — TRANSACTION COMPS: past M&A / PE deals for comparable companies and the multiples PAID. Use deals from an attached export if present and/or well-known DISCLOSED deals in the space. NEVER invent a deal value or multiple that was not actually disclosed — null any undisclosed multiple, and omit any "deal" you are not confident really happened.\n' +
